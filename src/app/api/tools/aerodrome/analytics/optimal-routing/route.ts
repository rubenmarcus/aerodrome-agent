...
import { getOptimalRouting } from '@/utils/graph';
...
export async function GET(request: Request) {
  try {
    ...
    if (directPool) {
      ...
      const reserveIn =
        directPool.token0.address === tokenIn
    ...
    for (const route of routes) {
      ...
      const amountIn = Number(amount);
      const reserveIn =
        firstPool.token0.address === tokenIn
      ...
      const reserveIn2 =
        secondPool.token0.address === route.intermediateToken
    ...
    return NextResponse.json({ error: 'Failed to get optimal routing' }, { status: 500 });
  }
}
